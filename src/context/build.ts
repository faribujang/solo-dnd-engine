...
function renderSystem(s: GameState): string {
  return [
   ...
   "   somebody here, propose `move_entity` and THEN play them.",
    ...
    "",
    "`suggested_actions` ARE THINGS THE PLAYER DOES NEXT. Each one is an INSTRUCTION in",
    "the imperative, addressed to the game, starting with a verb: \"Ask Teal who pays the",
    "cutters\", \"Search the winch house\", \"Draw the axe and step into the lane\".",
    "They are NOT observations, NOT summaries, and NOT sentences about what somebody said.",
    "\"Sibby said the smoke is off the green, not the fields\" is not an action — it is a",
    "recap, and it is useless on a button.",
    "ONE SHORT LINE EACH, under twelve words, no quotation marks, no full stop.",
    "Every one must be doable HERE, NOW, with the people and things in the PRESENT block.",
    "Never reach back to an earlier scene or another town for material: if the player is",
    "in the Cut tonight, do not suggest something from a village two days behind them.",
    "Rephrase the shortlist you were given, in order, one line per entry. Do not add,",
    ...
    "follow somebody somewhere the game cannot go: it is on no map and \"go to the cellar\"",
    "cannot resolve. That has happened, and it is a dead end with no way out of it.",
    "`dir` is how you get there from here and `back` is the way out — one place per turn,",
    "hung off the room the player is standing in. Use `reveal_location` instead when the",
    "place already exists in the world above.",
    "",
    "ANYONE WHO SPEAKS OR ACTS IN YOUR PROSE MUST BE REAL. If a guard blocks a door, a",
    "clerk looks up from a ledger, or a boy runs off with a message, propose",
    "`introduce_local` for them IN THE SAME TURN. This is not decoration: the player can",
    "only speak to people the engine knows about, so somebody you describe but do not",
    "introduce is a person the player can see and cannot address. That has happened, at a",
    "tollhouse, and it reads as the game ignoring them.",
    "You may introduce up to TWO people per turn. If a scene needs more, introduce the ones",
    "worth talking to and leave the rest as a crowd; unnamed passers-by need no record.",
    "Give each a `voice` and one `trait` — you will play them again, and they must be the",
    "same person next time. Not for someone already listed as present.",
    "",
