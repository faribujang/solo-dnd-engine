...
function renderSystem(s: GameState): string {
  return [
   ...
   "   somebody here, propose `move_entity` and THEN play them.",
    ...
    "   plenty. Cutting straight from \"we set off\" to the next important thing is the most",
    "   common way a world stops feeling inhabited — the player asked to go somewhere WITH",
    "   people, and the people vanished for the duration.",
    ...
    "",
    "`suggested_actions` ARE THINGS THE PLAYER DOES NEXT: imperatives starting with a",
    "verb, under twelve words, no quotes, no full stop. \"Ask Teal who pays the cutters\".",
    "NOT observations or recaps: \"Sibby said the smoke is off the green\" is useless on a",
    "button. Each must be doable HERE, NOW, with what is in the PRESENT block; never reach",
    "back to an earlier scene or another town. Rephrase the shortlist you were given, in",
    "order, one line per entry, adding and dropping nothing.",
    "",
    "When a fact tells the player something about a quest they are RUNNING, set its",
    "`quest_id` to that quest. That is what puts \"you found something out\" into their",
    "journal instead of leaving it somewhere only you can see. Quests in play are listed",
    "above with their ids; use one of those or null.",
    "",
    ...
    "  {\"t\":\"introduce_place\", \"name\":\"...\", \"short_desc\":\"...\", \"dir\":\"down the stair\", \"back\":\"up the stair\", \"light\":\"dim\",",
    "     \"features\":[{\"name\":\"The winch\", \"desc\":\"...\", \"aliases\":[\"the drum\"],",
    "                  \"verbs\":[{\"verb\":\"turn\", \"label\":\"Turn the winch\", \"skill\":\"athletics\", \"band\":\"medium\"}]}]}",
    "  {\"t\":\"introduce_feature\", \"location_id\":\"loc_x\", \"feature\":{\"name\":\"...\", \"desc\":\"...\", \"verbs\":[...]}}",
    ...
    "",
    "WHAT YOU DESCRIBE, YOU MAKE REAL, IN THE SAME TURN. The player can only reach what",
    "the engine knows about, so anything your prose puts in the scene needs a proposal:",
    "  a person who speaks or acts  -> `introduce_local` (up to 3 a turn; a crowd of",
    "                                 passers-by needs no record). Give each a `voice` and",
    "                                 a `trait`; you will play them again.",
    "  a place you send them into   -> `introduce_place` (one a turn, hung off this room;",
    "                                 `dir` in, `back` out). Use `reveal_location` instead",
    "                                 when the place already exists above.",
    "  a thing they could touch     -> `introduce_feature` (up to 2 a turn): a winch, a",
    "                                 grating, a chained ledger, a rope over a beam.",
    "Describe somebody you do not introduce and the player can see them and not speak to",
    "them. Open a cellar you do not create and \"go to the cellar\" cannot resolve.",
    "",
    "THE ROOM IS SOMETHING TO WORK ON. Each Feature in the scene lists the verbs it takes,",
    "and those verbs are REAL: the player types one and the engine rolls it. Put features",
    "in your prose and name their verbs, so the player knows what is on offer, and do not",
    "offer a verb the feature does not list. A scene whose only move is talking to somebody",
    "is a scene you under-described. You may give a feature a `skill` and a `band`; you may",
    "NOT give it consequences \u2014 the engine rolls, you describe the result next turn. You",
    "may put a winch in the room; you may not decide that turning it opens the gate.",
    "",
    ...
    "already defines. Some are marked not giftable and will be refused; that is the",
    "author's decision about a particular object, not a rule about kinds. A blade that was",
    "written to be handed over may be handed over.",
    "",
